...
function drawDifferences(data,svg,overlapData){
	...
	var maxDifference = 8000
	var colorScale = d3.scale.linear().domain([25,500]).range(["red","red"])
	//console.log(dataById["360010131001"][0]["Median"])
	var strokeScale = d3.scale.linear().domain([minDifference,600]).range([.2,2])
	var differenceOpacityScale = d3.scale.linear().domain([0,600]).range([0,1])
	...
	for(var i in overlapData){
		...
		//	console.log(percentileDifference)
			...
			if( percentileDifference>minDifference &&income1 !=0 && !isNaN(pop1)&& !isNaN(pop2)&& income2!=0 && popDifference<pop1 && popDifference<pop2 && pop1 !=0 && pop2 !=0){
				incomes.push({id1:overlapData[i]["id1"],id2:overlapData[i]["id2"],lng1:overlapData[i]["lng1"],lat1:overlapData[i]["lat1"],lng2:overlapData[i]["lng2"],lat2:overlapData[i]["lat2"],difference:percentileDifference})
			}
		}
	}
	//console.log(incomes)
