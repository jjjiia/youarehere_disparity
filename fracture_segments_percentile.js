...
function dataDidLoad(error, geojson1, city1, overlap, boroughs, neighbors,neighborhoodDictionary) {
	...
	initNycMap(geojson1, city1, "Median", "#svg-1",0,global.maxIncome*100000,boroughs,neighbors,overlap,neighborhoodDictionary)
...
function initNycMap(paths, data, column, svg,low,high,boroughs,neighbors,overlap,neighborhoodDictionary) {
...
function drawDifferences(data,svg,overlapData){
	...
	var colorScale = d3.scale.linear().domain([25,60]).range(["#aaa","red"])
	//console.log(dataById["360010131001"][0]["Median"])
	var strokeScale = d3.scale.linear().domain([minDifference,100]).range([0,2])
	var differenceOpacityScale = d3.scale.linear().domain([0,60]).range([0,1])
...
function formatDifferenceData(data,overlapData){
	...
	for(var i in overlapData){
		...
		//	console.log(percentileDifference)
			...
			//if( percentileDifference>minDifference &&income1 !=0 && !isNaN(pop1)&& !isNaN(pop2)&& income2!=0 && pop1 !=0 && pop2 !=0){
				incomes.push({income1:income1, income2:income2, percentile1:percentile1,percentile2:percentile2,id1:overlapData[i]["id1"],id2:overlapData[i]["id2"],lng1:overlapData[i]["lng1"],lat1:overlapData[i]["lat1"],lng2:overlapData[i]["lng2"],lat2:overlapData[i]["lat2"],difference:percentileDifference})
			}
		}
	}
	//console.log(incomes)
