...
function renderNycMap(data, column,svg,low,high,neighbors,neighborhoodDictionary) {
	...
	var colorScale = function(d) {
		...
		var x = companiesByZipcode[d.properties.geoid12]
		if(!x){
			return scale(1)
		}else{
			if(isNaN(x[0][column])) {
	...
	map.attr("stroke-opacity", 1)
		...
		map.on('mouseover', function(d){
			...
			var currentZipcode = d.properties.geoid12
		...
		//.on("click",function(d){
		//	console.log(d.properties.geoid12)
		//	console.log(companiesByZipcode[d.properties.geoid12][0]["Total"])
		//	console.log(companiesByZipcode[d.properties.geoid12])
		//})
	return map
}
